...
\begin{figure}
\begin{center}
\includegraphics[width=9.0cm]{Images/density_heating_panel}
\end{center}
...
Figure~\ref{fig:profiles_heating} shows the radial density and temperature profiles for the two runs (left and right set of panels), and Figure~\ref{fig:density_heating} shows the density slices for the same set of times and heating rates.  These plots demonstrate that a radiative heating source can substantially change the nature of the collapse, for a fixed metallicity ($Z=0.01 Z_\odot$ in this case).   In the mild heating, collapse proceeds in a roughly similar fashion to the no-heating run, except that the temperature does not fall as quickly, particularly in the center.  As the profiles show, gas inflow occurs driving high densiti
