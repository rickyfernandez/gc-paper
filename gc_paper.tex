...
by baryon mass such that a cell was refined whenever the its mass became larger than 0.1 M$_\odot$ as
well as a refinement based on the Jeans length such that the Jeans length was
always refined by 4 cells. 
...
$k_{\rm min}=$ 9 and $k_{\rm max} =$ 19 such that the input modes are well resolved
and have wavelengths smaller than the cloud radius.  We set the turbulent velocites
such that rms velocity of the gas is XXX km/s. Figure~\ref{fig:initial_setup} is a panel
of the initial number density, temperatue, velocity magnitude and pressure for all the
simulations in discussion.
\begin{figure}
\begin{center}
...
\end{center}
\caption{\label{fig:initial_setup}} Slices of inital number density, temperature, velocity magnitude
and pressure for all simulations in this paper. 
\end{figure}
...
\end{figure}
In the top hand panel of Figure~\ref{fig:profile_turbulence}, are density profiles given at multiple time steps for the $Z=10^{-3}Z_\odot$ run.
...
of the cloud begins to drive outward decreasing the amount of mass in each radii.
